...
1. 如果新切片长度超过两倍当前容量时：直接扩容到所需容量。
2. 如果所需容量小于256时：扩容为当前容量的两倍。
3. 容量大于等于256时：扩容为当前容量的1.25倍，并进行平滑过渡。
...
```go
func nextslicecap(){
	if newLen > doublecap  {扩容到所需容量}
	if oldCap < 256        {2 倍增长}
	for oldCap >= 256      {1.25 倍增长} 	
}
```
...
func nextslicecap(newLen, oldCap int) int {
	...
	for {
		// 3. 当 oldCap >= 256 时，newcap 1.25 倍增长
