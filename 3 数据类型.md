...
在 Go 中，当一个切片包含小于或等于 256 个元素时，它会每次按倍的容量增加，
当超过 256 个元素后，会按 25% 的容量增长，
所需容量超过两倍当前容量时，直接扩容到所需容量。

从小切片的 2 倍增长到大切片的 1.25 倍增长。这个公式在两者之间提供了平滑的过渡。

当切片需要扩容时，Go会根据以下规则调整容量：

- 容量小于256时：扩容为当前容量的两倍。
- 容量大于等于256时：扩容为当前容量的1.25倍，并进行平滑过渡。
- 所需容量超过两倍当前容量时：直接扩容到所需容量。
...
// growslice 为切片分配新的支撑存储
// arguments:
//
//	oldPtr = pointer to the slice's backing array 指向切片支撑数组的指针
...
func nextslicecap(newLen, oldCap int) int {
	...
	if newLen > doublecap {// 如果新切片长度超过两倍当前容量时：直接扩容到所需容量。
		return newLen
	}

	const threshold = 256 //阈值
	if oldCap < threshold {// 如果所需容量小于256时：扩容为当前容量的两倍。
		return doublecap
	}
	for {
