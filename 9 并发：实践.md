...
- 从上下文格式化一个键，返回一个格式化结构体的错误，在这两种情况下，格式化字符串都会导致：数据竞争和死锁
...
Go 的原生 map 不是同步容器：当并发写入或读写时，行为未定义，极易触发运行时 panic，如多个写操作并发执行，会出现数据竞态，甚至内存崩溃。
...
- 对分布在 不同 key 集 的写入操作支持更高并行效率，减少扩容冲突及缓存抖动
- Maps.Clone 性能回归问题
